...
class TidyPlanner {
    ...
    func generate(from image: UIImage, allowNetwork: Bool) async -> PlanResult {
        ...
        guard !filteredItems.isEmpty else {
            ...
            return PlanResult(plan: fallback, source: .local)
        }
        ...
        var plan = Plan(project: "TatsuTori Photo Split", locale: locale, tasks: tasks).validated()
        var source: PlanSource = .local

        guard FeatureFlags.intentSettingsV1 else {
            return PlanResult(plan: plan, source: source)
        }
        ...
        do {
            ...
            case .success(let outcome):
                tasks = outcome.tasks
                plan = Plan(project: "TatsuTori Photo Split", locale: locale, tasks: tasks).validated()
